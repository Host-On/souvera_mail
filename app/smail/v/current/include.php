...
if (!defined('SMAIL_DEV')) {
	define('SMAIL_DEV', '0.0.0' === APP_VERSION);
}
...
if (!defined('SMAIL_LIBRARIES_PATH')) {
	define('SMAIL_LIBRARIES_PATH', rtrim(realpath(__DIR__), '\\/').'/app/libraries/');

	if (false === set_include_path(SMAIL_LIBRARIES_PATH . PATH_SEPARATOR . get_include_path())) {
		exit('set_include_path() failed. Probably due to Apache config using php_admin_value instead of php_value');
	}

	spl_autoload_extensions('.php');
	/** lowercase autoloader (for non-Smail libraries) */
...
//		if ($file = stream_resolve_include_path(strtr($sClassName, '\\', DIRECTORY_SEPARATOR) . '.php')) {
	...
	/** Smail\Engine lowercase-filename autoloader
	 *  Handles engine classes that use lowercase file/directory names
	 *  e.g. Smail\Engine\Crypt → Smail/Engine/crypt.php
	 *       Smail\Engine\SASL\Plain → Smail/Engine/sasl/Plain.php (lowercase dir, PascalCase file)
	 */
	spl_autoload_register(function($sClassName){
		if (str_starts_with($sClassName, 'Smail\\Engine\\')) {
			...
			// e.g. Smail\Engine\SASL\XOAuth2 → Smail/Engine/sasl/XOAuth2.php
...
if (empty($_ENV['SMAIL_INCLUDE_AS_API'])) {
