...
//			, ['(USE (\Drafts \Sent))'] RFC 6154
	...
	public function FolderDelete(string $sFolderName) : void
	{
		...
		if ($oInfo->MESSAGES) {
			throw new \Smail\Mail\Client\Exceptions\NonEmptyFolder;
		}
				...
				if (\is_array($oResponse->OptionalResponse)) {
					...
					if (\count($oResponse->OptionalResponse) > 1) {
						if ('PERMANENTFLAGS' === $key && \is_array($oResponse->OptionalResponse[1])) {
							$oResult->PermanentFlags = \array_map('\\Smail\Mail\\Base\\Utils::Utf7ModifiedToUtf8', $oResponse->OptionalResponse[1]);
						}
...
//					$oResult->Flags = \array_map('\\Smail\Mail\\Base\\Utils::Utf7ModifiedToUtf8', $oResponse->ResponseList[2]);
				}
			}
		}
...
//				$aParameters[0] = '()';
			...
			if ($this->isImap4Rev2()) {
				\Smail\Engine\Log::warning('IMAP', 'IMAP4rev2 server without LIST-EXTENDED; LSUB fallback may fail');
			}
			$sCmd = 'LSUB';
		}
