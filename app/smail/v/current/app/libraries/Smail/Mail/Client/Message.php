...
//		$aFlags = [],
	...
	protected ?\Smail\Mail\Mime\EmailCollection
		$oFrom = null,
		$oSender = null,
		$oReplyTo = null,
		$oDeliveredTo = null,
	...
	private ?\Smail\Mail\Mime\HeaderCollection
		$Headers = null;
	...
	public function From() : ?\Smail\Mail\Mime\EmailCollection
	{
		return $this->oFrom;
	}
	...
	public static function fromFetchResponse(string $sFolder, \Smail\Mail\Imap\FetchResponse $oFetchResponse, ?\Smail\Mail\Imap\BodyStructure $oBodyStructure = null) : self
	{
		$oMessage = new self;
...
//		$oMessage->aFlags = $aFlags;
		$oMessage->aFlagsLowerCase = \array_map('mb_strtolower', \array_map('\\Smail\Mail\\Base\\Utils::Utf7ModifiedToUtf8', $aFlags));
		$oMessage->iInternalTimeStampInUTC = \Smail\Mail\Base\DateTimeHelper::ParseInternalDateString(
...
//		$oMessage->sEmailId = $oMessage->sEmailId ?: $oFetchResponse->GetFetchValue('X-GUID');
		...
		$oHeaders = \strlen($sHeaders) ? new \Smail\Mail\Mime\HeaderCollection($sHeaders, $sCharset) : null;
		if ($oHeaders) {
			$oMessage->Headers = $oHeaders;

			$sContentTypeCharset = $oHeaders->ParameterValue(
				MimeHeader::CONTENT_TYPE->value,
				\Smail\Mail\Mime\Enumerations\Parameter::CHARSET->value
			);
			...
			$oMessage->iHeaderTimeStampInUTC = \Smail\Mail\Base\DateTimeHelper::ParseRFC2822DateString(
...
//			$oMessage->sDeliveryReceipt = \trim($oHeaders->ValueByName(MimeHeader::RETURN_RECEIPT_TO->value));
			...
			if ($sReadReceipt) {
				try
				{
					if (!\Smail\Mail\Mime\Email::Parse($sReadReceipt)) {
						$sReadReceipt = '';
					}
				}
			...
			if (\strlen($sDraftInfo)) {
				...
				$oParameters = new \Smail\Mail\Mime\ParameterCollection($sDraftInfo);
		...
		{
			$sCharset = $sCharset ?: \Smail\Mail\Base\Enumerations\Charset::ISO_8859_1->value;
...
*/
			...
			if ($aTextParts) {
				$sCharset = $sCharset ?: \Smail\Mail\Base\Enumerations\Charset::UTF_8->value;
				...
				foreach ($aTextParts as $oPart) {
					...
					if (\is_string($sText) && \strlen($sText)) {
						...
						$sText = Utils::ConvertEncoding($sText,
							...
							\Smail\Mail\Base\Enumerations\Charset::UTF_8->value
						);
			...
			if ($gAttachmentsParts->valid()) {
				...
				foreach ($gAttachmentsParts as /* @var $oAttachmentItem \Smail\Mail\Imap\BodyStructure */ $oAttachmentItem) {
...
*/
		$result = array(
			...
			// \Smail\Mail\Mime\EmailCollection
			'from' => $this->oFrom,
