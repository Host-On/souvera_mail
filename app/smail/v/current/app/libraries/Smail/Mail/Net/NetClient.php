...
abstract class NetClient
{
	...
	{
		throw new \Smail\Mail\RuntimeException($sErrStr, $iErrNo);
	}

	/**
	 * @throws \ValueError
	 * @throws \Smail\Mail\RuntimeException
	 * @throws \Smail\Mail\Net\Exceptions\SocketAlreadyConnectedException
	 * @throws \Smail\Mail\Net\Exceptions\SocketCanNotConnectToHostException
	 */
...
//			$this->Disconnect();
		...
		$this->ssl = \Smail\Mail\Net\Enumerations\ConnectionSecurityType::UseSSL($this->Settings->port, $this->Settings->type);
...
//			$this->sConnectedHost = ($this->ssl ? 'ssl://' : '') . $this->sConnectedHost;
		...
		if (!$this->ssl && \Smail\Mail\Net\Enumerations\ConnectionSecurityType::SSL->value === $this->Settings->type) {
			$this->writeLogException(
				new \Smail\Mail\Net\Exceptions\SocketUnsuppoterdSecureConnectionException('SSL isn\'t supported: ('.\implode(', ', \stream_get_transports()).')'),
				\LOG_ERR);
		}
		...
		if (!$this->rConnect) {
			$this->writeLogException(
				new Exceptions\SocketCanNotConnectToHostException(
					\Smail\Mail\Base\Locale::ConvertSystemString($sErrorStr), (int) $iErrorNo,
		...
		if ($this->rConnect && \Smail\Mail\Base\Utils::FunctionCallable('stream_set_timeout')) {
	...
	{
		$bSuccess = false;
		if ($this->rConnect && \Smail\Mail\Base\Utils::FunctionCallable('stream_socket_enable_crypto')) {
...
//			}
		...
		$bSuccess || $this->writeLogException(new \Smail\Mail\RuntimeException('Cannot enable STARTTLS.'), \LOG_ERR);
	}
...
//	abstract public function Login(ConnectSettings $oSettings) : self;
	...
	{
		return $this->rConnect && \Smail\Mail\Base\Utils::FunctionCallable('stream_context_get_options')
			? \stream_context_get_params($this->rConnect) : false;
	}

	/**
	 * @throws \Smail\Mail\Net\Exceptions\SocketConnectionDoesNotAvailableException
	 * @throws \Smail\Mail\Net\Exceptions\SocketWriteException
	 */
	...
	/**
	 * @throws \Smail\Mail\Net\Exceptions\SocketConnectionDoesNotAvailableException
	 * @throws \Smail\Mail\Net\Exceptions\SocketReadException
	 */
	...
	protected function assertEncryptedForBearerAuth() : void
	{
		if (!$this->Encrypted()) {
			...
			if (!\in_array($sHost, array('127.0.0.1', '::1', 'localhost'), true)) {
				$this->writeLogException(new \Smail\Mail\RuntimeException(
