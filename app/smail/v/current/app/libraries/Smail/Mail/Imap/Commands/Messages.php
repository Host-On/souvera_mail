...
namespace Smail\Mail\Imap\Commands;

use Smail\Mail\Imap\FetchResponse;
use Smail\Mail\Imap\Enumerations\FetchType;
use Smail\Mail\Imap\ResponseCollection;
use Smail\Mail\Imap\SequenceSet;
use Smail\Mail\Imap\Enumerations\MessageFlag;
use Smail\Mail\Imap\Enumerations\ResponseType;
...
*/
	...
	private function writeMessageStream($rMessageStream) : ?int
	{
		...
		\Smail\Mail\Base\Utils::WriteStream($rMessageStream, $this->ConnectionResource());
...
*/
	 ...
	 * @throws \InvalidArgumentException
	 * @throws \Smail\Mail\RuntimeException
	 * $sStoreAction = \Smail\Mail\Imap\Enumerations\StoreAction::ADD_FLAGS_SILENT->value
	 */
	public function MessageStoreFlag(SequenceSet $oRange, array $aInputStoreItems, string $sStoreAction) : ?ResponseCollection
	{
		...
		$aInputStoreItems = \array_map('\\Smail\Mail\\Base\\Utils::Utf8ToUtf7Modified', $aInputStoreItems);
	...
	{
		$oSort = new \Smail\Mail\Imap\Requests\SORT($this);
	...
	{
		$oESearch = new \Smail\Mail\Imap\Requests\ESEARCH($this);
...
//		if (!$this->UTF8 && !\mb_check_encoding($sSearchCriterias, 'UTF-8')) {
		if (!$this->UTF8 && !\Smail\Mail\Base\Utils::IsAscii($sSearchCriterias)) {
...
//		if (!$this->UTF8 && !\mb_check_encoding($sSearchCriterias, 'UTF-8')) {
	...
	{
		$oThread = new \Smail\Mail\Imap\Requests\THREAD($this);
