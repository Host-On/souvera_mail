...
namespace Smail\Mail\Smtp;

use Smail\Mail\Net\Enumerations\ConnectionSecurityType;
use Smail\Engine\IDN;
 ...
 * @package Smtp
 */
class SmtpClient extends \Smail\Mail\Net\NetClient
{
	/** @var Settings */
	public \Smail\Mail\Net\ConnectSettings $Settings;
	 ...
	 */
	public function Connect(\Smail\Mail\Net\ConnectSettings $oSettings) : void
	{
	...
	private function StartTLS() : void
	{
		if ($this->hasCapability('STARTTLS')) {
			...
			$this->writeLogException(
				new \Smail\Mail\Net\Exceptions\SocketUnsuppoterdSecureConnectionException('STARTTLS is not supported'),
				\LOG_ERR);
		}
	}
	...
	public function Login(Settings $oSettings) : self
	{
		...
		foreach ($oSettings->SASLMechanisms as $sasl_type) {
			if (\in_array(\strtoupper($sasl_type), $this->aAuthTypes) && \Smail\Engine\SASL::isSupported($sasl_type)) {
				$type = $sasl_type;
				break;
			}
		}
		if (!$type) {
			...
			$this->writeLogException(new \Smail\Mail\Smtp\Exceptions\LoginBadMethodException);
		}

		$this->assertEncryptedForBearerAuth();

		$SASL = \Smail\Engine\SASL::factory($type);
		$sResult = '';
		...
		{
			$this->writeLogException(
				new \Smail\Mail\Smtp\Exceptions\LoginBadMethodException($oException->GetResponses(), $oException->getMessage(), 0, $oException)
			);
		}
		...
		{
			$this->writeLogException(
				new \Smail\Mail\Smtp\Exceptions\LoginBadCredentialsException($oException->GetResponses(), $oException->getMessage(), 0, $oException)
			);
		}
...
//			$sCmd .= ' BODY=BINARYMIME';
	...
	{
		if (!$this->bMail) {
			$this->writeLogException(new \Smail\Mail\RuntimeException('No sender reverse path has been supplied'), \LOG_ERR);
		}
...
//		$sTo = IDN::emailToAscii($sTo);
	...
	public function Data(string $sData) : self
	{
		...
		$rDataStream = \Smail\Mail\Base\ResourceRegistry::CreateMemoryResourceFromString($sData);
		unset($sData);
		$this->DataWithStream($rDataStream);
		\Smail\Mail\Base\ResourceRegistry::CloseMemoryResource($rDataStream);
	...
	public function DataWithStream($rDataStream) : self
	{
		...
		if (!$this->bRcpt) {
			$this->writeLogException(new \Smail\Mail\RuntimeException('No recipient forward path has been supplied'), \LOG_ERR);
		}
...
/*
		...
		if ($this->hasCapability('CHUNKING')) {
			...
			while (!\feof($rDataStream)) {
				...
				if (false === $sBuffer) {
					if (!\feof($rDataStream)) {
						$this->writeLogException(new \Smail\Mail\RuntimeException('Cannot read input resource'), \LOG_ERR);
					}
					break;
				}
				$this->sendRequestWithCheck("BDAT " . \strlen($sBuffer) . "\r\n{$sBuffer}", 250);
				\Smail\Mail\Base\Utils::ResetTimeLimit();
			}
...
*/
	...
	{
		$sUser = \Smail\Mail\Base\Utils::Trim($sUser);
/*
		// RFC 6531
		if ($this->hasCapability('SMTPUTF8')) {
...
*/
	...
	/**
	 * @throws \Smail\Mail\Smtp\Exceptions\ResponseException
	 */
