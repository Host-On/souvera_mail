...
class ESEARCH extends Request
{
	...
	function __construct(\Smail\Mail\Imap\ImapClient $oImapClient)
	{
		if (!$oImapClient->hasCapability('ESEARCH')) {
			$oImapClient->writeLogException(new \Smail\Mail\RuntimeException('ESEARCH is not supported'), \LOG_ERR);
		}
...
/*		// RFC 6203
		if (false !== \stripos($this->sCriterias, 'FUZZY') && !$this->oImapClient->hasCapability('SEARCH=FUZZY')) {
			$this->oImapClient->writeLogException(new \Smail\Mail\RuntimeException('SEARCH=FUZZY is not supported'), \LOG_ERR);
		}
*/
		...
		if ($aFolders) {
			if (!$this->oImapClient->hasCapability('MULTISEARCH')) {
				$this->oImapClient->writeLogException(new \Smail\Mail\RuntimeException('MULTISEARCH is not supported'), \LOG_ERR);
			}
			$sCmd = 'ESEARCH';
			$aReques[] = 'IN';
			$aReques[] = $aFolders;
			...
			if (!$this->oImapClient->hasCapability('CONTEXT=SEARCH')) {
				foreach ($this->aReturn as $sReturn) {
					if (\preg_match('/PARTIAL|UPDATE|CONTEXT/i', $sReturn)) {
						$this->oImapClient->writeLogException(new \Smail\Mail\RuntimeException('CONTEXT=SEARCH is not supported'), \LOG_ERR);
					}
				}
			}
