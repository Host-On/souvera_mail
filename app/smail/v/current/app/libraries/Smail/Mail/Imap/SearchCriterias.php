...
class SearchCriterias
{
	...
	*/
	...
	public static function fromString(\Smail\Mail\Imap\ImapClient $oImapClient, string $sFolderName, string $sSearch, bool $bHideDeleted, bool &$bUseCache = true) : self
	{
		$iTimeFilter = 0;
			...
			if (!$aLines) {
				...
				foreach ($aLines as $sName => $sRawValue) {
					...
					switch ($sName) {
						case 'FROM':
						case 'SUBJECT':
						case 'BODY': // $sValue = \trim(\Smail\Mail\Base\Utils::StripSpaces($sValue), '"');
						...
						case 'KEYWORD':
							...
							$aCriteriasResult[] = static::escapeSearchString(
								$oImapClient,
								\Smail\Mail\Base\Utils::Utf8ToUtf7Modified($sRawValue)
							);
							break;
...
//			$aCriteriasResult[] = \gmdate('j-M-Y', $iTimeFilter);
	...
	public static function escapeSearchString(\Smail\Mail\Imap\ImapClient $oImapClient, string $sSearch) : string
	{
//		return $oImapClient->EscapeString($sSearch);
//		return \Smail\Mail\Base\Utils::IsAscii($sSearch) || $oImapClient->hasCapability('QQMail'))
		return (\Smail\Mail\Base\Utils::IsAscii($sSearch) || !$oImapClient->hasCapability('LITERAL+'))
	...
	{
		if (\strlen($sDate)) {
			$oDateTime = \DateTime::createFromFormat('Y-m-d', $sDate, \Smail\Mail\Base\DateTimeHelper::GetUtcTimeZoneObject());
	...
	/**
	 * Smail search like: 'from=foo&to=test&is[]=unseen&is[]=flagged'
	 */
	...
	private static function parseSearchString(string $sSearch) : array
	{
		...
		$sSearch = \Smail\Mail\Base\Utils::StripSpaces($sSearch);
		...
		if (\preg_match_all('/".*?(?<!\\\)"/', $sSearch, $mMatch)) {
			foreach ($mMatch[0] as $sItem) {
				do {
					$sKey = \Smail\Mail\Base\Utils::Sha1Rand();
				}
