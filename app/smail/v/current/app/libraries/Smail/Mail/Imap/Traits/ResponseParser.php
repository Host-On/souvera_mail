...
namespace Smail\Mail\Imap\Traits;

use Smail\Mail\Imap\Response;
use Smail\Mail\Imap\Enumerations\ResponseType;
use Smail\Mail\Imap\Exceptions\ResponseNotFoundException;
...
//									$this->writeLog('{'.$iLiteralLen.'} '.$sLiteral, \LOG_INFO);
									...
									\Smail\Engine\Log::notice('IMAP', 'Literal treated as atom and skipped');
								}
								unset($sLiteral);
							} else {
				...
				case '"':
					...
					while (true) {
						...
						case "\r":
						case "\n":
							\Smail\Engine\Log::notice('IMAP', 'Invalid char in quoted string: "' . \substr($this->sResponseBuffer, $iPos, $iOffset + $iLength - $iPos) . '"');
...
//				\set_time_limit(10);
	...
	private function partialResponseLiteralCallbacks(?string $sParent, ?string $sLiteralAtomUpperCase, int $iLiteralLen) : bool
	{
		...
		$rImapLiteralStream = \Smail\Mail\Base\StreamWrappers\Literal::CreateStream($this->ConnectionResource(), $iLiteralLen);
