...
namespace Smail\Mail\Imap\Requests;
...
class SORT extends Request
{
	...
	function __construct(\Smail\Mail\Imap\ImapClient $oImapClient)
	{
		if (!$oImapClient->hasCapability('SORT')) {
			$oImapClient->writeLogException(new \Smail\Mail\RuntimeException('SORT is not supported'), \LOG_ERR);
		}
	...
	{
		if (!$this->aSortTypes) {
			$this->oImapClient->writeLogException(new \Smail\Mail\RuntimeException('SortTypes are missing'), \LOG_ERR);
		}
		...
		if ($this->aReturn) {
			...
			if (!$this->oImapClient->hasCapability('ESORT')) {
				$this->oImapClient->writeLogException(new \Smail\Mail\RuntimeException('ESORT is not supported'), \LOG_ERR);
			}
			if (!$this->oImapClient->hasCapability('CONTEXT=SORT')) {
				foreach ($this->aReturn as $sReturn) {
					if (\preg_match('/PARTIAL|UPDATE|CONTEXT/i', $sReturn)) {
						$this->oImapClient->writeLogException(new \Smail\Mail\RuntimeException('CONTEXT=SORT is not supported'), \LOG_ERR);
					}
				}
			}
			$aRequest[] = 'RETURN';
		...
		$aRequest[] = 'UTF-8'; // \strtoupper(\Smail\Mail\Base\Enumerations\Charset::UTF_8)
