...
class FetchResponse
{
	...
	public function GetFetchEnvelopeEmailCollection(int $iIndex, string $sParentCharset = \Smail\Mail\Base\Enumerations\Charset::ISO_8859_1->value) : ?\Smail\Mail\Mime\EmailCollection
	{
		$oResult = null;
		$aEmails = $this->GetFetchEnvelopeValue($iIndex);
		if (\is_array($aEmails) && \count($aEmails)) {
			$oResult = new \Smail\Mail\Mime\EmailCollection;
			foreach ($aEmails as $aEmailItem) {
				if (\is_array($aEmailItem) && 4 === \count($aEmailItem)) {
					$sDisplayName = \Smail\Mail\Base\Utils::DecodeHeaderValue(
						self::findEnvelopeIndex($aEmailItem, 0, ''), $sParentCharset);

//					$sRemark = \Smail\Mail\Base\Utils::DecodeHeaderValue(
//						self::findEnvelopeIndex($aEmailItem, 1, ''), $sParentCharset);
					...
					if (\strlen($sLocalPart) && \strlen($sDomainPart)) {
						$oResult->append(
							new \Smail\Mail\Mime\Email($sLocalPart.'@'.$sDomainPart, $sDisplayName)
						);
					}
				}
			}
