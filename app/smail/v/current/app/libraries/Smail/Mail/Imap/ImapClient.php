 ...
 * @package Imap
 */
class ImapClient extends \Smail\Mail\Net\NetClient
{
	...
	/**
	 * Used by \Smail\Mail\Client\MailClient::MessageMimeStream
	 */
...
//	public function Connect(Settings $oSettings) : void
	...
	public function Login(Settings $oSettings) : self
	{
		...
		foreach ($oSettings->SASLMechanisms as $sasl_type) {
			if ($this->hasCapability("AUTH={$sasl_type}") && \Smail\Engine\SASL::isSupported($sasl_type)) {
				$type = $sasl_type;
				break;
			}
		}
		if (!$type) {
			...
			throw new \Smail\Mail\RuntimeException('No supported SASL mechanism found, remote server wants: '
...
//						\Smail\Engine\Log::warning('IMAP', "{$oResponse->OptionalResponse[0]}: {$this->lastCommand}");
					}
...
*/
	...
	{
		return $this->EscapeString($this->UTF8 ? $sFolderName : \Smail\Mail\Base\Utils::Utf8ToUtf7Modified($sFolderName));
	}

	public function toUTF8(string $sText) : string
	{
		return $this->UTF8 ? $sText : \Smail\Mail\Base\Utils::Utf7ModifiedToUtf8($sText);
	}
}
