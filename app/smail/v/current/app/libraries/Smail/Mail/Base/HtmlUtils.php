...
*/
		...
		@$oDoc->loadHTML('<?xml version="1.0" encoding="utf-8"?>'.
			...
			'<body '.\trim($sBodyAttrs).'>'.\Smail\Mail\Base\Utils::Utf8Clear($sHtml).'</body></html>');

		$oDoc->normalizeDocument();

		if (\Smail\Mail\Base\Utils::FunctionCallable('libxml_clear_errors')) {
			\libxml_clear_errors();
		}
	...
	{
		$sText = \Smail\Mail\Base\Utils::StripSpaces($sText);
