		...
		if ('message/rfc822' !== \strtolower($this->ContentType())) {
			$oAttachmentPart->Headers->append(
				new Header(
					...
					\Smail\Mail\Base\Enumerations\Encoding::BASE64_LOWER->value
				)
			);

			if (\is_resource($oAttachmentPart->Body) && !\Smail\Mail\Base\StreamWrappers\Binary::IsStreamRemembed($oAttachmentPart->Body)) {
				$oAttachmentPart->Body =
					\Smail\Mail\Base\StreamWrappers\Binary::CreateStream($oAttachmentPart->Body,
						\Smail\Mail\Base\StreamWrappers\Binary::GetInlineDecodeOrEncodeFunctionName(
							\Smail\Mail\Base\Enumerations\Encoding::BASE64_LOWER->value, false));

				\Smail\Mail\Base\StreamWrappers\Binary::RememberStream($oAttachmentPart->Body);
			}
		}
