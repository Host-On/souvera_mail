...
*/
	...
	{
		$sReferences = \Smail\Mail\Base\Utils::StripSpaces($sReferences);
	...
	private function generateNewMessageId(string $sHostName = '') : string
	{
		...
		if (empty($sHostName) && \Smail\Mail\Base\Utils::FunctionCallable('php_uname')) {
		...
		return '<'.
			\Smail\Mail\Base\Utils::Sha1Rand($sHostName.
				(\Smail\Mail\Base\Utils::FunctionCallable('getmypid') ? \getmypid() : '')).'@'.$sHostName.'>';
	}
				...
				if (1 === \count($aAttachments) && isset($aAttachments[0])) {
					...
					if ($resource = $aAttachments[0]->Resource()) {
						if (\is_resource($resource)) {
							...
							$oPart->Body = \Smail\Mail\Base\ResourceRegistry::CreateMemoryResourceFromString($resource);
						}
					}
	...
	public function ToStream(bool $bWithoutBcc = false)
	{
		...
		if ($this->bAddDefaultXMailer && !isset($this->aHeadersValue[Enumerations\Header::X_MAILER->value])) {
			$oRootPart->Headers->SetByName(Enumerations\Header::X_MAILER->value, 'Smail/'.APP_VERSION, true);
		}
