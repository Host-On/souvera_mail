...
use Smail\Mail\Imap\Responses\ACL as ACLResponse;
...
*/
	...
	private function FolderACLRequest(string $sFolderName, string $sCommand, array $aParams) : \Smail\Mail\Imap\ResponseCollection
	{
...
//			\array_unshift($aParams, $this->EscapeFolderName($sFolderName));
		...
		return new \Smail\Mail\Imap\ResponseCollection();
	}
	...
	public function FolderGetACL(string $sFolderName) : array
	{
		...
		if ($response->hasRight('a')) {
			...
			foreach ($oResponses as $oResponse) {
				// * ACL INBOX.shared demo@smail.dev akxeilprwtscd demo2@smail.dev lrwstipekxacd
				if (\Smail\Mail\Imap\Enumerations\ResponseType::UNTAGGED->value === $oResponse->ResponseType
