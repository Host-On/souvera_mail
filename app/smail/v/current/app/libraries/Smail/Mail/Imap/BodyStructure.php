...
use Smail\Mail\Mime\Enumerations\ContentType;
...
//		return 'application/pgp-encrypted' !== $this->sContentType
	...
	public function GetHtmlAndPlainParts() : array
	{
		 ...
		 * Only when \Smail\Engine\Api::Config()->Get('security', 'openpgp', true)
		 */
		if (!$aParts) {
	...
	public function SearchByCallback(callable $fCallback, /*BodyStructure*/ $parent = null) : \Generator
	{
		...
		foreach ($this->aSubParts as /* @var $oSubPart \Smail\Mail\Imap\BodyStructure */ $oSubPart) {
...
//			return $oItem->IsAttachment();
	...
	private static function decodeAttrParameter(array $aParams, string $sParamName, string $sCharset) : string
	{
		...
		if (isset($aParams[$sParamName])) {
			$sResult = \Smail\Mail\Base\Utils::DecodeHeaderValue($aParams[$sParamName], $sCharset);
		} else if (isset($aParams[$sParamName.'*'])) {
			$aValueParts = \explode("''", $aParams[$sParamName.'*'], 2);
			if (2 === \count($aValueParts)) {
				$sCharset = isset($aValueParts[0]) ? $aValueParts[0] : \Smail\Mail\Base\Enumerations\Charset::UTF_8->value;
				$sResult = \Smail\Mail\Base\Utils::ConvertEncoding(
					\urldecode($aValueParts[1]), $sCharset, \Smail\Mail\Base\Enumerations\Charset::UTF_8->value);
			} else {
			...
			if (\count($aFileNames)) {
				...
				if (\strlen($sCharset)) {
					$sResult = \Smail\Mail\Base\Utils::ConvertEncoding($sResult,
						$sCharset, \Smail\Mail\Base\Enumerations\Charset::UTF_8->value);
				}
			}
		}
	...
	public static function NewInstance(array $aBodyStructure, string $sPartID = '') : ?self
	{
		...
		$oStructure->sFileName = \Smail\Mail\Base\Utils::Utf8Clear($sFileName);
...
//			'mimeTypeParams' => $this->aContentTypeParams,
			'fileName' => \Smail\Mail\Base\Utils::SecureFileName($this->FileName(true)),
