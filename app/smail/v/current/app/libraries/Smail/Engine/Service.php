...
//		\header('Permissions-Policy: interest-cohort=()');
		...
		if ($oConfig->Get('security', 'force_https', false) && !$oHttp->IsSecure()) {
			\Smail\Mail\Base\Http::Location('https://'.$oHttp->GetHost(false).$oHttp->GetUrl());
			return true;
		}
		...
		if (\count($aPaths) && !empty($aPaths[0]) && 'index' !== \strtolower($aPaths[0])) {
			if ('mailto' !== \strtolower($aPaths[0]) && !\Smail\Engine\HTTP\SecFetch::matchAnyRule($oConfig->Get('security', 'secfetch_allow', ''))) {
				\Smail\Mail\Base\Http::StatusHeader(403);
				echo $oServiceActions->ErrorTemplates('Access Denied.',
					"Disallowed Sec-Fetch
					Dest: " . ($_SERVER['HTTP_SEC_FETCH_DEST'] ?? '') . "
					Mode: " . ($_SERVER['HTTP_SEC_FETCH_MODE'] ?? '') . "
					Site: " . ($_SERVER['HTTP_SEC_FETCH_SITE'] ?? '') . "
					User: " . (\Smail\Engine\HTTP\SecFetch::user() ? 'true' : 'false'));
				return false;
			}
...
//			if (!\Smail\Engine\HTTP\SecFetch::isEntering()) {
			...
			$aTemplateParameters = array(
				...
				'{{LoadingDescriptionEsc}}' => \htmlspecialchars($oConfig->Get('webmail', 'loading_description', 'Smail'), ENT_QUOTES|ENT_IGNORE, 'UTF-8'),
				'{{BaseAppAdmin}}' => 0
			);
...
//			$oActions->verifyCacheByKey($sCacheFileName);
			...
			$sScriptNonce = \Smail\Engine\UUID::generate();
