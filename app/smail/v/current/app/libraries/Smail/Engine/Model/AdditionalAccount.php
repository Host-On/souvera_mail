...
class AdditionalAccount extends Account
{
	...
	{
		return \Smail\Engine\IDN::emailToAscii(\Smail\Engine\Api::Actions()->getMainAccountFromToken()->Email());
	}
	...
	public function asTokenArray(MainAccount $oMainAccount) : array
	{
		...
		$aData['pass'] = \Smail\Engine\Crypt::EncryptUrlSafe($aData['pass'], $sHash); // sPassword
		if (!empty($aData['smtp']['pass'])) {
			$aData['smtp']['pass'] = \Smail\Engine\Crypt::EncryptUrlSafe($aData['smtp']['pass'], $sHash);
		}
		...
		if (!empty($aAccountHash['email'])) {
			...
			if ($sPasswordHMAC) {
				if ($sPasswordHMAC === \hash_hmac('sha1', $aAccountHash['pass'], $sHash)) {
					$aAccountHash['pass'] = \Smail\Engine\Crypt::DecryptUrlSafe($aAccountHash['pass'], $sHash);
					if (!empty($aAccountHash['smtp']['pass'])) {
						$aAccountHash['smtp']['pass'] = \Smail\Engine\Crypt::DecryptUrlSafe($aAccountHash['smtp']['pass'], $sHash);
					}
				} else {
					$aAccountHash['pass'] = '';
					if (!empty($aAccountHash['smtp']['pass'])) {
