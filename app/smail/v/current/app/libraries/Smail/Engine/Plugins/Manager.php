...
class Manager
{
	...
	private \Smail\Engine\Actions $oActions;
	...
	public function __construct(\Smail\Engine\Actions $oActions)
	{
		...
		if ($this->bIsEnabled) {
			...
			if (\strlen($sList)) {
				$aList = \Smail\Engine\Repository::getEnabledPackagesNames();
	...
	public function CreatePluginByName(string $sName) : ?\Smail\Engine\Plugins\AbstractPlugin
	{
		$oPlugin = null;
		...
		if ($sClassName) {
			...
			$oPlugin
				...
				->SetPluginConfig(new \Smail\Engine\Config\Plugin($sName, $oPlugin->ConfigMap(true)))
			;
	...
	public function loadPluginByName(string $sName) : ?string
	{
		if (\preg_match('/^[a-z0-9\\-]+$/', $sName)) {
			...
			if (\class_exists($sClassName) && \is_subclass_of($sClassName, 'Smail\\Engine\\Plugins\\AbstractPlugin')) {
				return $sClassName;
			}
	...
	public function Actions() : \Smail\Engine\Actions
	{
	...
	public function InitAppData(bool $bAdmin, array &$aAppData, ?\Smail\Engine\Model\Account $oAccount = null) : self
	{
		if ($this->bIsEnabled && isset($aAppData['Plugins']) && \is_array($aAppData['Plugins'])) {
			...
			foreach ($this->aPlugins as $oPlugin) {
				if ($oPlugin) {
					...
					if (\is_array($aMap)) {
						foreach ($aMap as /* @var $oPluginProperty \Smail\Engine\Plugins\Property */ $oPluginProperty) {
