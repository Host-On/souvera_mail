...
abstract class AbstractPlugin
{
	const
		NAME     = '',
		AUTHOR   = 'Smail',
		URL      = 'https://github.com/PhiGi87/souvera_mail',
		VERSION  = '0.0',
		RELEASE  = '2020-11-01',
		REQUIRED = '2.0.0',
//		DEPRECATED = '',
	...
	/**
	 * @var \Smail\Engine\Config\Plugin
	 */
	...
	final public function Config() : \Smail\Engine\Config\Plugin
	{
		return $this->oPluginConfig;
	}

	final public function Manager() : \Smail\Engine\Plugins\Manager
	{
	...
	final public function ConfigMap(bool $flatten = false) : array
	{
		...
		if ($flatten) {
			...
			foreach ($this->aConfigMap as $oItem) {
				if ($oItem) {
					if ($oItem instanceof \Smail\Engine\Plugins\Property) {
						$result[] = $oItem;
					} else if ($oItem instanceof \Smail\Engine\Plugins\PropertyCollection) {
						foreach ($oItem as $oSubItem) {
							if ($oSubItem && $oSubItem instanceof \Smail\Engine\Plugins\Property) {
								$result[] = $oSubItem;
							}
						}
					}
	...
	final public function SetPluginManager(\Smail\Engine\Plugins\Manager $oPluginManager) : self
	{
	...
	final public function SetPluginConfig(\Smail\Engine\Config\Plugin $oPluginConfig) : self
	{
