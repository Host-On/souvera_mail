...
class Actions
{
	...
	/**
	 * This session cookie contains a \Smail\Engine\Model\Account
	 * Value is Base64 EncryptToJSON
	 */
	const AUTH_SPEC_TOKEN_KEY = 'x2maccount';

	/**
	 * This session cookie optionally contains a \Smail\Engine\Model\AdditionalAccount
	 * Value is Base64 EncryptToJSON
	 */
	...
	/**
	 * @var \Smail\Mail\Client\MailClient
	 */
	...
	/**
	 * @var \Smail\Mail\Log\Logger
	 */
	protected $oLoggerAuth;

	/**
	 * @var array of \Smail\Mail\Cache\CacheClient
	 */
	protected $aCachers = array();

	/**
	 * @var \Smail\Engine\Providers\Storage
	 */
	...
	/**
	 * @var \Smail\Engine\Providers\Files
	 */
	protected $oFilesProvider = null;

	/**
	 * @var \Smail\Engine\Providers\Domain
	 */
	protected $oDomainProvider = null;

	/**
	 * @var \Smail\Engine\Providers\Settings
	 */
	...
	function __construct()
	{
		...
		if ($this->oConfig->Get('logs', 'enable', false) || $this->oConfig->Get('debug', 'enable', false)) {
			...
			if ('syslog' === $sLogFileName) {
				$oDriver = new \Smail\Mail\Log\Drivers\Syslog();
			} else if ('stderr' === $sLogFileName) {
				$oDriver = new \Smail\Mail\Log\Drivers\StderrStream();
			} else {
				...
				$oDriver = new \Smail\Mail\Log\Drivers\File($sLogFileFullPath . '/' . $this->compileLogFileName($sLogFileName));
			}
			...
			$this->logWrite(
				...
				. '][PID:' . (\Smail\Mail\Base\Utils::FunctionCallable('getmypid') ? \getmypid() : 'unknown')
				. '][' . \Smail\Mail\Base\Http::GetServer('SERVER_SOFTWARE', '~')
				. '][' . \PHP_SAPI
				. '][Streams:' . \implode(',', \stream_get_transports())
				. '][' . $oHttp->GetMethod() . ' ' . $oHttp->GetScheme() . '://' . $oHttp->GetHost(false) . \Smail\Mail\Base\Http::GetServer('REQUEST_URI', '') . ']'
			);
		}
		...
		if (null === $mResult) {
			switch ($sName) {
				...
				case 'address-book':
					...
					// No standalone fallback — Smail requires Nextcloud
					break;
				case 'identities':
				case 'suggestions':
					$mResult = [];
...
/*				// See function Cacher
				case 'cache':
					$mResult = new \Smail\Mail\Cache\Drivers\File(
...
/*
		try {
			if (!\Smail\Engine\Shutdown::count() && $this->ImapClient()->IsLoggined()) {
...
*/
		...
		if (false !== \strpos($sLine, '{request:')) {
			...
			if (false !== \strpos($sLine, '{request:domain-clear}')) {
				$sLine = \str_replace('{request:domain-clear}',
					\Smail\Mail\Base\Utils::GetClearDomainName($this->Http()->GetHost(true, true)),
					$sLine);
			}
		...
		if (false !== \strpos($sLine, '{user:')) {
			...
			if (\preg_match('/\{user:(email|login|domain)\}/i', $sLine)) {
				...
				if ($oAccount) {
					...
					$sLine = \str_replace('{user:login}', \Smail\Mail\Base\Utils::getEmailAddressLocalPart($sEmail), $sLine);
					$sLine = \str_replace('{user:domain}', \Smail\Mail\Base\Utils::getEmailAddressDomain($sEmail), $sLine);
					$sLine = \str_replace('{user:domain-clear}',
						\Smail\Mail\Base\Utils::GetClearDomainName(\Smail\Mail\Base\Utils::getEmailAddressDomain($sEmail)),
						$sLine);
				}
			}
	...
	public function Http(): \Smail\Mail\Base\Http
	{
		if (null === $this->oHttp) {
			$this->oHttp = \Smail\Mail\Base\Http::SingletonInstance();
		}

		return $this->oHttp;
	}

	public function MailClient(): \Smail\Mail\Client\MailClient
	{
		if (null === $this->oMailClient) {
			$this->oMailClient = new \Smail\Mail\Client\MailClient();
	...
	public function ImapClient(): \Smail\Mail\Imap\ImapClient
	{
//		$this->initMailClientConnection();
	 ...
	 */
	public function Cacher(?Model\Account $oAccount = null, bool $bForceFile = false): \Smail\Mail\Cache\CacheClient
	{
		$sKey = '';
		if ($oAccount) {
		...
		if (!isset($this->aCachers[$sIndexKey])) {
			$this->aCachers[$sIndexKey] = new \Smail\Mail\Cache\CacheClient();

			$oDriver = $bForceFile ? null : $this->fabrica('cache');
			if (!($oDriver instanceof \Smail\Mail\Cache\DriverInterface)) {
				$oDriver = new \Smail\Mail\Cache\Drivers\File(
					\trim($this->oConfig->Get('cache', 'path', '')) ?: APP_PRIVATE_DATA . 'cache'
				);
			}
	...
	{
		if ($sLogin) {
			$sHost = $admin ? $this->Http()->GetHost(true, true) : \Smail\Mail\Base\Utils::getEmailAddressDomain($sLogin);
			$aAdditionalParams = array(
				...
				'{smtp:host}' => $sHost,
				'{user:email}' => $sLogin,
				'{user:login}' => $admin ? $sLogin : \Smail\Mail\Base\Utils::getEmailAddressLocalPart($sLogin),
				'{user:domain}' => $sHost,
			);
		} else {
			$aAdditionalParams = array();
		...
		if (!empty($sLine)) {
			if (!$this->oLoggerAuth) {
				$this->oLoggerAuth = new \Smail\Mail\Log\Logger(false);
...
//					$this->oLoggerAuth->SetLevel(\LOG_WARNING);
					...
					$this->oLoggerAuth->append(
						(new \Smail\Mail\Log\Drivers\File($sAuthLogFileFullPath))
							->DisableTimePrefix()
							->DisableGuidPrefix()
							->DisableTypedPrefix()
					);
	...
	public function AppData(bool $bAdmin): array
	{
		...
		$aResult = array(
			'Auth' => false,
			'title' => $oConfig->Get('webmail', 'title', 'Smail'),
			'loadingDescription' => $oConfig->Get('webmail', 'loading_description', 'Smail'),
			'Plugins' => array(),
			'System' => array(
				'version' => APP_VERSION,
				'token' => Utils::GetCsrfToken(),
				'languages' => \Smail\Engine\L10n::getLanguages(false),
				'webPath' => \Smail\Engine\Utils::WebPath(),
				'webVersionPath' => \Smail\Engine\Utils::WebVersionPath()
			),
		);
		...
		if ($oAccount) {
				...
				$sToken = \Smail\Engine\Cookies::get(self::AUTH_MAILTO_TOKEN_KEY);
				if (null !== $sToken) {
					\Smail\Engine\Cookies::clear(self::AUTH_MAILTO_TOKEN_KEY);

					$mMailToData = Utils::DecodeKeyValuesQ($sToken);
					if (!empty($mMailToData['MailTo']) && 'MailTo' === $mMailToData['MailTo'] && !empty($mMailToData['To'])) {
						$aResult['mailToEmail'] = \Smail\Engine\IDN::emailToUtf8($mMailToData['To']);
					}
				}
...
//				}
	...
	public function Upload(?array $aFile, int $iError): array
	{
		...
		if ($oAccount && UPLOAD_ERR_OK === $iError && \is_array($aFile)) {
			...
			// Detect content-type
			$type = \Smail\Engine\File\MimeType::fromFile($aFile['tmp_name'], $aFile['name'])
				?: \Smail\Engine\File\MimeType::fromFilename($aFile['name']);
			if ($type) {
				$aFile['type'] = $type;
				$sSavedName .= \Smail\Engine\File\MimeType::toExtension($type);
			}
	...
	public function Capa(bool $bAdmin, ?Model\Account $oAccount = null): array
	{
		...
		if (!$aResult) {
			...
			$aResult = array(
				...
				Capa::GNUPG->value               => (bool) $oConfig->Get('security', 'gnupg', true) && \Smail\Engine\PGP\GnuPG::isSupported(),
...
//		if ($sKey && $this->oConfig->Get('cache', 'enable', true) && $this->oConfig->Get('cache', 'http', true)) {
	...
	{
		if ($sKey && $this->oConfig->Get('cache', 'enable', true) && $this->oConfig->Get('cache', 'http', true)) {
			\Smail\Mail\Base\Http::ServerUseCache(
				$this->etag($sKey),
	...
	{
		if ($sKey && $this->oConfig->Get('cache', 'enable', true) && $this->oConfig->Get('cache', 'http', true)) {
			\Smail\Mail\Base\Http::checkETag($this->etag($sKey));
//			\Smail\Mail\Base\Http::checkLastModified(0);
		}
	}
	...
	public function encodeRawKey(array $aValues): string
	{
		...
		return \Smail\Mail\Base\Utils::UrlSafeBase64Encode(\json_encode($aValues));
	}

	public function decodeRawKey(string $sRawKey): array
	{
		return empty($sRawKey) ? []
			: (\json_decode(\Smail\Mail\Base\Utils::UrlSafeBase64Decode($sRawKey), true) ?: []);
/*
...
*/
	...
	public function Location(string $sUrl, int $iStatus = 302): void
	{
		...
		\Smail\Mail\Base\Http::Location($sUrl, $iStatus);
	}
