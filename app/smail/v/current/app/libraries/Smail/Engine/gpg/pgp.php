...
namespace Smail\Engine\GPG;

use Smail\Engine\SensitiveString;

class PGP extends Base implements \Smail\Engine\PGP\PGPInterface
{
	private
		$_message,
...
//		\rewind($fp);
	...
	{
//		\Smail\Engine\Log::debug('GnuPG', "export({$fingerprint}, {$passphrase})");
...
//			'NO_SECKEY' => [],
	...
	private function exec(array $arguments, bool $throw = true) /*: array|false*/
	{
		if (\version_compare($this->version, '2.2.5', '<')) {
			\Smail\Engine\Log::error('GPG', "{$this->version} too old");
			return false;
		}
