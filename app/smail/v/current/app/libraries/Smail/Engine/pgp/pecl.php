...
class PECL implements \Smail\Engine\PGP\PGPInterface
{
	private
		$homedir,
		// Instance of gnupg pecl extension https://www.php.net/gnupg
		$GnuPG,
		// Instance of \Smail\Engine\GPG\PGP
		$GPG;
...
*/
	...
	{
//		\Smail\Engine\Log::debug('GnuPG', "addDecryptKey({$fingerprint}, {$passphrase})");
	...
	{
//		\Smail\Engine\Log::debug('GnuPG', "addSignKey({$fingerprint}, {$passphrase})");
...
//		\rewind($fp);
	...
	public function verify(string $signed_text, string $signature, ?string &$plaintext = null) /*: array|false*/
	{
		...
		if (!$result) {
			\Smail\Engine\Log::notice('GnuPG', 'gnupg_verify() failed: ' . $this->GnuPG->geterror());
			\Smail\Engine\Log::info('GnuPG', \print_r($this->GnuPG->geterrorinfo(),1));
		}
		return $result;
	}
