...
trait Localization
{
	...
	{
		$sResult = \Smail\Engine\L10n::validLanguage($sLanguage, $bAdmin)
		?: \Smail\Engine\L10n::validLanguage($sDefault, $bAdmin);
		...
		return \Smail\Engine\L10n::validLanguage($sResult, $bAdmin) ? $sResult : 'en';
	}
	...
	public function detectClientLanguage(bool $bAdmin): string
	{
		...
		$sAcceptLang = \strtolower(\Smail\Mail\Base\Http::GetServer('HTTP_ACCEPT_LANGUAGE', 'en'));
	...
	public function StaticI18N(string $sKey): string
	{
		...
		if (null === $aLang) {
			...
			$aLang = \Smail\Engine\L10n::load($sLang, 'static');
	...
	public function compileLanguage(string $sLanguage, bool $bAdmin = false) : string
	{
		...
		$langs = \array_flip(\Smail\Engine\L10n::getLanguages($bAdmin));
		...
		$aResultLang = \array_replace_recursive(
			$aResultLang,
			\Smail\Engine\L10n::load($sLanguage, ($bAdmin ? 'admin' : 'user'))
		);
