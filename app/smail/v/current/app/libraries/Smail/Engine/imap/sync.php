...
namespace Smail\Engine\Imap;

use Smail\Mail\Imap\Enumerations\FetchType;
use Smail\Mail\Imap\Enumerations\MessageFlag;
use Smail\Mail\Mime\Enumerations\Header;

class Sync
{
	public \Smail\Mail\Imap\ImapClient $oImapSource;
	public \Smail\Mail\Imap\ImapClient $oImapTarget;
...
//		$this->oImapTarget->logWrite('Get oImapTarget->FolderList');
		\Smail\Engine\Log::notice('SYNC', 'Get oImapTarget->FolderList');
		...
		\Smail\Engine\Log::notice('SYNC', 'Get oImapSource->FolderList');
		...
		if ($isCli) {
			...
			\Smail\Engine\HTTP\Stream::start();
			\Smail\Engine\HTTP\Stream::JSON([
...
/*
							...
							if ($iAppendUid && $aFlags) {
								$this->MessageStoreFlag(
									...
									$aFlags,
									\Smail\Mail\Imap\Enumerations\StoreAction::ADD_FLAGS_SILENT
								);
							}
*/
						}
