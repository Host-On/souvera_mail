...
use Smail\Mail\Imap\SequenceSet;
use Smail\Mail\Imap\Enumerations\FetchType;
use Smail\Mail\Imap\Enumerations\MessageFlag;
use Smail\Mail\Mime\Part as MimePart;
use Smail\Mail\Mime\Enumerations\Header as MimeEnumHeader;
...
//			\ignore_user_abort(true);
	...
	public function DoSaveMessage() : array
	{
		...
		$mResult = false;
		if ($oMessage) {
			$rMessageStream = \Smail\Mail\Base\ResourceRegistry::CreateMemoryResource();

			$iMessageStreamSize = \Smail\Mail\Base\Utils::WriteStream($oMessage->ToStream(false), $rMessageStream, 8192, true);
...
/*
		...
		if ($aAuth) {
			...
			$oAccount->setSmtpPass(new \Smail\Engine\SensitiveString($aAuth['password']));
...
*/
		...
		try
		{
			if ($oMessage) {
				...
				$iMessageStreamSize = \Smail\Mail\Base\Utils::WriteStream(
				...
				if (false !== $iMessageStreamSize) {
					...
					if (\strlen($sSaveFolder)) {
						...
						{
							if ($oMessage->GetBcc()) {
								$rAppendMessageStream = \Smail\Mail\Base\ResourceRegistry::CreateMemoryResource();
								$iAppendMessageStreamSize = \Smail\Mail\Base\Utils::WriteStream(
		...
		catch (\Smail\Mail\Smtp\Exceptions\LoginBadCredentialsException $oException)
		{
		...
		try
		{
			if ($oMessage && $this->AddressBookProvider($oAccount)->IsActive()) {
				...
				if ($oToCollection) {
					foreach ($oToCollection as /* @var $oEmail \Smail\Mail\Mime\Email */ $oEmail) {
			...
			if ($oMessage) {
				...
				if (false !== $iMessageStreamSize) {
					...
					if (\strlen($sFolderFullName) && 0 < $iUid) {
						...
						{
							$this->Cacher($oAccount)->Set(\Smail\Engine\KeyPathHelper::ReadReceiptCache($oAccount->Email(), $sFolderFullName, $iUid), '1');
						}
					}
				}
			}
...
//			$accountHash = $aValues[3];
		...
		try
		{
			...
			if ($oMessage->smimeSigned && ($bAutoVerify || !$oMessage->smimeSigned['detached'])) try {
				...
				$result = (new \Smail\Engine\SMime\OpenSSL(''))->verify($sBody, null, $bOpaque);
				if ($result) {
					if ($bOpaque) {
							...
							$this->MailClient()->MessageMimeStream(
								function($rResource, $sContentType, $sFileName, $sMimeIndex = '') use ($oAccount, $sTempName, $self, &$aAttachments, $mIndex) {
									if (\is_resource($rResource)) {
										$sContentType =
											$sContentType
											?: \Smail\Engine\File\MimeType::fromStream($rResource, $sFileName)
											?: \Smail\Engine\File\MimeType::fromFilename($sFileName)
											?: 'application/octet-stream'; // 'text/plain'

										$sTempName .= \Smail\Engine\File\MimeType::toExtension($sContentType);
...
//												'size' => 0
							...
							$sContentType = \Smail\Engine\File\MimeType::fromStream($rResource, $sTempName)
								?: \Smail\Engine\File\MimeType::fromFilename($sTempName)
								?: 'application/octet-stream'; // 'text/plain'
							$aAttachments[$mIndex] = [
//								'name' => '',
								'tempName' => $sTempName,
...
//								'size' => $oFilesProvider->FileSize($oAccount, $sTempName)
	...
	/**
	 * @throws \Smail\Engine\Exceptions\ClientException
	 * @throws \Smail\Mail\Net\Exceptions\ConnectionException
	 */
	private function smtpSendMessage(Account $oAccount, \Smail\Mail\Mime\Message $oMessage,
		...
		bool $bDsn = false, bool $bRequireTLS = false)
	{
		...
		try
		{
			...
			$sFrom = $oFrom instanceof \Smail\Mail\Mime\Email ? $oFrom->GetEmail() : '';
			...
			$oSmtpClient = new \Smail\Mail\Smtp\SmtpClient();
			...
			if ($oSmtpClient->IsConnected()) {
				...
				foreach ($oRcpt as /* @var $oEmail \Smail\Mail\Mime\Email */ $oEmail) {
		...
		catch (\Smail\Mail\Net\Exceptions\ConnectionException $oException)
		{
		...
		catch (\Smail\Mail\Smtp\Exceptions\LoginException $oException)
		{
			throw new ClientException(Notifications::AuthError->value, $oException);
		}
		catch (\Throwable $oException)
	...
	private function buildReadReceiptMessage(Account $oAccount) : \Smail\Mail\Mime\Message
	{
		...
		$oMessage = new \Smail\Mail\Mime\Message();

		if ($this->Config()->Get('security', 'hide_x_mailer_header', true)) {
			$oMessage->DoesNotAddDefaultXMailer();
		}

		$oMessage->SetFrom(new \Smail\Mail\Mime\Email($oIdentity->Email(), $oIdentity->Name()));
		...
		if (!empty($sReplyTo)) {
			$oReplyTo = new \Smail\Mail\Mime\EmailCollection($sReplyTo);
		...
		$oToEmails = new \Smail\Mail\Mime\EmailCollection($sReadReceipt);
		...
		$oPart->Body = \Smail\Mail\Base\StreamWrappers\Binary::CreateStream(
			\Smail\Mail\Base\ResourceRegistry::CreateMemoryResourceFromString(\preg_replace('/\\r?\\n/su', "\r\n", \trim($sText))),
	 ...
	 */
	private function buildMessage(Account $oAccount, bool $bWithDraftInfo = true) : \Smail\Mail\Mime\Message
	{
		$oMessage = new \Smail\Mail\Mime\Message();
		...
		$oMessage->SetFrom(\Smail\Mail\Mime\Email::Parse($this->GetActionParam('from', '')));
		$oFrom = $oMessage->GetFrom();

		$oMessage->RegenerateMessageId($oFrom ? $oFrom->GetDomain() : '');

		$oMessage->SetReplyTo(new \Smail\Mail\Mime\EmailCollection($this->GetActionParam('replyTo', '')));
...
//			$oMessage->SetReadReceipt(($oIdentity ?: $oAccount)->Email());
		...
		if (!empty($this->GetActionParam('markAsImportant', 0))) {
			$oMessage->SetPriority(\Smail\Mail\Mime\Enumerations\MessagePriority::HIGH->value);
		}

		$oMessage->SetSubject($this->GetActionParam('subject', ''));

		$oMessage->SetTo(new \Smail\Mail\Mime\EmailCollection($this->GetActionParam('to', '')));
		$oMessage->SetCc(new \Smail\Mail\Mime\EmailCollection($this->GetActionParam('cc', '')));
		$oMessage->SetBcc(new \Smail\Mail\Mime\EmailCollection($this->GetActionParam('bcc', '')));
...
//			$sProtocol = $match[1][0];
			...
			$sHtml = \Smail\Mail\Base\HtmlUtils::BuildHtml($sHtml, $aFoundCids, $aFoundDataURL, $aFoundContentLocationUrls);
			...
			// First add plain
			$sPlain = $this->GetActionParam('plain', '') ?: \Smail\Mail\Base\HtmlUtils::ConvertHtmlToPlain($sHtml);
			...
			$oAlternativePart->Body = \Smail\Mail\Base\StreamWrappers\Binary::CreateStream(
				\Smail\Mail\Base\ResourceRegistry::CreateMemoryResourceFromString(\preg_replace('/\\r?\\n/su', "\r\n", \trim($sHtml))),
				'convert.quoted-printable-encode'
			);
...
/*
			if ($sSignature = $this->GetActionParam('pgpSignature', null)) {
				...
				$oAlternativePart->Body = \Smail\Mail\Base\ResourceRegistry::CreateMemoryResourceFromString(\preg_replace('/\\r?\\n/su', "\r\n", \trim($sPlain)));
				$oPart->SubParts->append($oAlternativePart);
			...
			foreach ($aAttachments as $sTempName => $aData) {
				...
				if (\is_resource($rResource)) {
					...
					$oMessage->Attachments()->append(
						new \Smail\Mail\Mime\Attachment($rResource, $sFileName, $iFileSize, $bIsInline,
...
/*
		...
		if ($sPublicKey) {
			$oMessage->Attachments()->append(
				new \Smail\Mail\Mime\Attachment(
...
*/
		foreach ($aFoundDataURL as $sCidHash => $sDataUrlString) {
			...
			{
				$sRaw = \Smail\Mail\Base\Utils::Base64Decode($aMatch[2]);
				$iFileSize = \strlen($sRaw);
				if (0 < $iFileSize) {
					$sFileName = \preg_replace('/[^a-z0-9]+/i', '.', $aMatch[1]);
					$rResource = \Smail\Mail\Base\ResourceRegistry::CreateMemoryResourceFromString($sRaw);
					...
					$oMessage->Attachments()->append(
						new \Smail\Mail\Mime\Attachment($rResource, $sFileName, $iFileSize, true, true, $sCID)
					);
				}
			}
		}

		$oPassphrase = new \Smail\Engine\SensitiveString($this->GetActionParam('signPassphrase', ''));
...
*/
			...
			\Smail\Mail\Base\StreamFilters\LineEndings::appendTo($resource);
...
*/
			if ($sCertificate && $sPrivateKey) {
				...
				$resource = $oBody->ToStream();
				\Smail\Mail\Base\StreamFilters\LineEndings::appendTo($resource);
				$tmp = new \Smail\Engine\File\Temporary('mimepart');
