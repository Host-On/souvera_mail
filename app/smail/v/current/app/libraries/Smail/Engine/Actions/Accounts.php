...
use Smail\Engine\Model\MainAccount;
use Smail\Engine\Model\AdditionalAccount;
use Smail\Engine\Model\Identity;
use Smail\Engine\Notifications;
use Smail\Engine\Providers\Identities;
...
trait Accounts
{
	private ?\Smail\Engine\Providers\Identities $oIdentitiesProvider = null;
	...
	public function GetAccounts(MainAccount $oAccount): array
	{
		if ($this->GetCapa(Capa::ADDITIONAL_ACCOUNTS->value)) {
			...
			$aAccounts = $sAccounts ? \json_decode($sAccounts, true)
				: \Smail\Engine\Upgrade::ConvertInsecureAccounts($this, $oAccount);
	...
	public function DoAccountSetup(): array
	{
		...
		$oPassword = new \Smail\Engine\SensitiveString($this->GetActionParam('password', ''));
		$bNew = !empty($this->GetActionParam('new', 1));

		if ($bNew || \strlen($oPassword)) {
			/** @var \Smail\Engine\Model\AdditionalAccount $oNewAccount */
			...
			$aAccount = \Smail\Engine\Model\AdditionalAccount::convertArray($aAccounts[$sEmail]);
		}
	...
	protected function loadAdditionalAccountImapClient(string $sEmail): \Smail\Mail\Imap\ImapClient
	{
		...
		$oImapClient = new \Smail\Mail\Imap\ImapClient;
...
//				$this->SetAdditionalAuthToken(null);
				\Smail\Engine\Cookies::clear(self::AUTH_ADDITIONAL_TOKEN_KEY);
				$bReload = true;
			}
...
//			'Email' => IDN::emailToUtf8($oAccount->Email()),
			...
			'mainEmail' => \Smail\Engine\Api::Actions()->getMainAccountFromToken()->Email(),
...
/*		// TODO: verify private key for certificate?
		if ($oIdentity->smimeCertificate && $oIdentity->smimeKey) {
			new \Smail\Engine\SMime\Certificate($oIdentity->smimeCertificate, $oIdentity->smimeKey);
		}
*/
