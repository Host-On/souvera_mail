...
use Smail\Mail\Cache\CacheClient;
...
class DefaultDomain implements DomainInterface
{
	...
	{
//		return ('*' === $sName) ? 'default' : \str_replace('*', '_wildcard_', \Smail\Engine\IDN::toAscii($sName));
...
//		$aList = \glob($this->sDomainPath.'/*.{ini,json,alias}', GLOB_BRACE);
	...
	{
//		$sName = \Smail\Engine\IDN::toAscii($sName);
		...
		if (\file_exists($sRealFileBase.'.json')) {
			...
			return \Smail\Engine\Model\Domain::fromArray($sName, $aDomain);
		}

		if (\file_exists($sRealFileBase.'.ini')) {
			$aDomain = \parse_ini_file($sRealFileBase.'.ini') ?: array();
			return \Smail\Engine\Model\Domain::fromIniArray($sName, $aDomain);
		}
		...
		if ($bFindWithWildCard) {
			...
			$sFoundValue = '';
			if (\strlen($sNames)
			 && \Smail\Engine\Plugins\Helper::ValidateWildcardValues($sName, $sNames, $sFoundValue)
			 && \strlen($sFoundValue)
			) {
	...
	public function Save(\Smail\Engine\Model\Domain $oDomain) : bool
	{
		...
		\Smail\Engine\Utils::saveFile("{$this->sDomainPath}/{$sRealFileName}.json", \json_encode($oDomain, \JSON_PRETTY_PRINT));
	...
	{
//		$sTarget = \Smail\Engine\IDN::toAscii($sTarget);
...
//		$this->Delete($sName);
		\Smail\Engine\Utils::saveFile("{$this->sDomainPath}/{$sRealFileName}.alias", $sTarget);
		$this->oCacher && $this->oCacher->Delete(static::CACHE_KEY);
		return true;
	}
	...
	protected function getDisabled() : array
	{
		...
		$sItem = \strtok($sFile, ",\n");
		while (false !== $sItem) {
//			$aDisabled[] = \Smail\Engine\IDN::toAscii($sItem);
...
//		$sName = \Smail\Engine\IDN::toAscii($sName);
		...
		if ($sName) {
			...
			\Smail\Engine\Utils::saveFile($this->sDomainPath.'/disabled', \implode("\n", $aResult));
