...
trait UserAuth
{
	...
	{
		$sEmail = \Smail\Engine\IDN::emailToAscii(\Smail\Mail\Base\Utils::Trim($sEmail));
		...
		$sSmtpUser = $sEmail;
		if (\str_contains($sEmail, '@')
		 && ($oDomain || ($oDomain = $oDomainProvider->Load(\Smail\Mail\Base\Utils::getEmailAddressDomain($sEmail), true)))
		) {
...
//				,new SensitiveString($oPassword)
	...
	public function switchAccount(string $sEmail) : bool
	{
		...
		if ($sEmail && $oMainAccount && $this->GetCapa(Capa::ADDITIONAL_ACCOUNTS->value)) {
			...
			if ($oMainAccount->Email() !== $sEmail) {
				$sEmail = \Smail\Engine\IDN::emailToAscii($sEmail);
	...
	/**
	 * Returns Smail\Engine\Model\AdditionalAccount when it exists,
	 * else returns Smail\Engine\Model\MainAccount when it exists,
	 * else null
	 *
	 * @throws \Smail\Engine\Exceptions\ClientException
	 */
...
//		Cookies::clear(Utils::SESSION_TOKEN);
	 ...
	 * @throws \Smail\Engine\Exceptions\ClientException
	 */
	protected function imapConnect(Account $oAccount, bool $bAuthLog = false, ?\Smail\Mail\Imap\ImapClient $oImapClient = null): void
	{
		try {
			if (!$oImapClient) {
		...
		} catch (\Smail\Mail\Imap\Exceptions\LoginBadCredentialsException $oException) {
			if ($bAuthLog) {
