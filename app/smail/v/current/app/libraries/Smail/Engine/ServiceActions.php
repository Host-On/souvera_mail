...
{
	protected \Smail\Mail\Base\Http $oHttp;
	...
	public function __construct(\Smail\Mail\Base\Http $oHttp, Actions $oActions)
	{
		$this->oHttp = $oHttp;
		$this->oActions = $oActions;
	}

	private function Logger() : \Smail\Mail\Log\Logger
	{
	...
	private function Cacher() : \Smail\Mail\Cache\CacheClient
	{
...
//						$this->oActions->logMask($value);
		...
		{
			\Smail\Engine\Log::warning('SERVICE', "{$oException}");
			if ($e = $oException->getPrevious()) {
				\Smail\Engine\Log::warning('SERVICE', "- {$e}");
			}
...
//				\is_uploaded_file($_FILES[$sInputName]['tmp_name'])
	...
	public function ServiceProxyExternal() : string
	{
		...
		if ($sData && $this->Config()->Get('labs', 'use_local_proxy_for_external_images', false)) {
			...
			$sUrl = \Smail\Mail\Base\Utils::UrlSafeBase64Decode($sData);
			if (!empty($sUrl)) {
				\header('X-Content-Location: '.$sUrl);
				$tmp = \tmpfile();
				$HTTP = \Smail\Engine\HTTP\Request::factory();
				...
				if ($oResponse) {
					$sContentType = \Smail\Engine\File\MimeType::fromStream($tmp) ?: $oResponse->getHeader('content-type');
					if (200 === $oResponse->status && \str_starts_with($sContentType, 'image/')) {
						try {
							...
							\Smail\Engine\Log::error('Proxy', \get_class($HTTP) . ': ' . $e->getMessage());
						}
					} else {
		...
		\Smail\Mail\Base\Http::StatusHeader(404);
		return '';
	}

	public function ServiceCspReport() : void
	{
		\Smail\Engine\HTTP\CSP::logReport();
	}
...
//					error_log(print_r($e,1));
	...
	{
		$this->oHttp->ServerNoCache();
		\Smail\Engine\Integrity::test();
		return '';
	}

	/**
	 * Login with the \Smail\Engine\API::CreateUserSsoHash() generated hash
	 */
	public function ServiceSso() : string
	{
		...
		if (!empty($sSsoHash)) {
			...
			if (!empty($sSsoSubData)) {
				$aData = \Smail\Engine\Crypt::DecryptFromJSON($sSsoSubData, $sSsoHash);
					...
					(\time() - 10 < $aData['Time']))
				{
					...
					try
					{
						$oAccount = $this->oActions->LoginProcess(
							\trim($aData['Email']),
							new \Smail\Engine\SensitiveString($aData['Password'])
						);
