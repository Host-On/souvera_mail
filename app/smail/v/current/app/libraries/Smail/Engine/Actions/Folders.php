...
use Smail\Mail\Imap\Enumerations\FolderType;
...
trait Folders
{
	...
	public function DoFolderAppend(): array
	{
		...
		if (\UPLOAD_ERR_OK != $_FILES['appendFile']['error']) {
			...
			return $this->FalseResponse($iErrorCode, \Smail\Engine\Enumerations\UploadError::getMessage($iErrorCode));
		}
...
//			FolderInformation(string $sFolderName, int $iPrevUidNext = 0, array $aUids = array())
	...
	public function DoFolderDelete() : array
	{
		...
		catch (\Smail\Mail\Client\Exceptions\NonEmptyFolder $oException)
		{
	...
	public function DoFolderInformation() : array
	{
		...
		try
		{
			return $this->DefaultResponse($this->MailClient()->FolderInformation(
				...
				new \Smail\Mail\Imap\SequenceSet($this->GetActionParam('flagsUids', []))
			));
		}
		catch (\Throwable $oException)
		{
	...
	{
//		$oImapClient->FolderSetACL('INBOX', 'demo@smail.dev', 'lrwstipekxacd');
//		$oImapClient->FolderSetACL($sFolderFullName, 'demo@smail.dev', 'lrwstipekxacd');
//		$oImapClient->FolderSetACL($sFolderFullName, 'foobar@smail.dev', 'lr');
		$this->initMailClientConnection();
