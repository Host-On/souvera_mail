...
use Smail\Engine\Providers\Suggestions;
...
trait User
{
	...
	public function DoLogin() : array
	{
		try {
			$oAccount = $this->LoginProcess(
				\Smail\Mail\Base\Utils::Trim($this->GetActionParam('Email', '')),
				new \Smail\Engine\SensitiveString($this->GetActionParam('Password', ''))
			);
	...
	public function DoAppDelayStart() : array
	{
		...
		$sTimers = $this->StorageProvider()->Get(null,
			\Smail\Engine\Providers\Storage\Enumerations\StorageType::NOBODY->value, 'Cache/Timers', '');
...
//			$oSettingsLocal->SetConf('Theme', $this->ValidateTheme($oConfig->Get('webmail', 'theme', 'Default')));
		...
		$this->setSettingsFromParams($oSettings, 'Layout', 'int', function ($iValue) {
			return (int) (\in_array((int) $iValue, array(\Smail\Engine\Enumerations\Layout::NO_PREVIEW->value,
				\Smail\Engine\Enumerations\Layout::SIDE_PREVIEW->value, \Smail\Engine\Enumerations\Layout::BOTTOM_PREVIEW->value)) ?
					$iValue : \Smail\Engine\Enumerations\Layout::SIDE_PREVIEW->value);
		});
	...
	private function setSettingsFromParams(\Smail\Engine\Settings $oSettings, string $sConfigName, string $sType = 'string', ?callable $cCallback = null) : void
	{
