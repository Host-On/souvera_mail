...
abstract class Log
{
	...
	{
		if (\Smail\Engine\Api::Logger()->IsEnabled()) {
			\Smail\Engine\Api::Logger()->Write($msg, $level, $prefix);
		} else {
/*
			static $log_level;
			// Default to level 4, 0 = LOG_EMERG, 7 = LOG_DEBUG
			if (!$log_level) {
				$log_level = \max(3, \Smail\Engine\Api::Config()->Get('logs', 'level', \LOG_WARNING));
			}
			if ($level <= $log_level) {
				if (\Smail\Engine\Api::Config()->Get('logs', 'syslog') && \openlog('smail', \LOG_ODELAY, \LOG_USER)) {
...
//					\error_log($prefix . ' ' . static::$levels[$level] . ': ' . $msg, 3, 'filename');
			...
			if (\class_exists('OC')) {
//				\OCP\Log\logger('smail')->log(\intval($level), $msg);
				\OCP\Log\logger('smail')->{static::$levels[$level]}($msg);
			}
*/
		}
	}
