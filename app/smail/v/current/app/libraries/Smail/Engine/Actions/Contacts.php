...
{
	/**
	 * @var \Smail\Engine\Providers\AddressBook
	 */
	protected $oAddressBookProvider = null;

	public function AddressBookProvider(?\Smail\Engine\Model\Account $oAccount = null): \Smail\Engine\Providers\AddressBook
	{
...
//				if ($this->oConfig->Get('contacts', 'enable', false)) {
				...
				\Smail\Engine\Log::error('AddressBook', $e->getMessage()."\n".$e->getTraceAsString());
				$oDriver = null;
//				$oDriver = new \Smail\Engine\Providers\AddressBook\PdoAddressBook();
			}
			$this->oAddressBookProvider = new \Smail\Engine\Providers\AddressBook($oDriver);
		...
		if ($this->HasActionParam('uid') && $this->HasActionParam('jCard')) {
			...
			if ($oAddressBookProvider && $oAddressBookProvider->IsActive()) {
				...
				if ($vCard && $vCard instanceof \Sabre\VObject\Component\VCard) {
					...
					$vCard->PRODID = 'Smail-'.APP_VERSION;
					$sUid = \trim($this->GetActionParam('uid'));
					$oContact = $sUid ? $oAddressBookProvider->GetContactByID($sUid) : null;
					if (!$oContact) {
						$oContact = new \Smail\Engine\Providers\AddressBook\Classes\Contact();
					}
	...
	public function UploadContacts(?array $aFile, int $iError) : array
	{
		...
		if ($oAccount && UPLOAD_ERR_OK === $iError && \is_array($aFile)) {
			...
			if (!$this->FilesProvider()->MoveUploadedFile($oAccount, $sSavedName, $aFile['tmp_name'])) {
				$iError = \Smail\Engine\Enumerations\UploadError::ON_SAVING->value;
			} else {
		...
		if (UPLOAD_ERR_OK !== $iError) {
			$iClientError = 0;
			$sError = \Smail\Engine\Enumerations\UploadError::getUserMessage($iError, $iClientError);
			if (!empty($sError)) {
	...
	private function importContactsFromVcfFile(\Smail\Engine\Model\Account $oAccount, /*resource*/ $rFile): int
	{
		$iCount = 0;
		$oAddressBookProvider = $this->AddressBookProvider($oAccount);
		if (\is_resource($rFile) && $oAddressBookProvider && $oAddressBookProvider->IsActive()) {
			try
			{
				...
				foreach (\Smail\Engine\Providers\AddressBook\Utils::VcfStreamToContacts($rFile) as $oContact) {
	...
	private function importContactsFromCsvFile(\Smail\Engine\Model\Account $oAccount, /*resource*/ $rFile, string $sFileStart): int
	{
		$iCount = 0;
		$oAddressBookProvider = $this->AddressBookProvider($oAccount);
		if (\is_resource($rFile) && $oAddressBookProvider && $oAddressBookProvider->IsActive()) {
			try
			{
				...
				foreach (\Smail\Engine\Providers\AddressBook\Utils::CsvStreamToContacts($rFile, $sDelimiter) as $oContact) {
					if ($oAddressBookProvider->ContactSave($oContact)) {
						++$iCount;
					}
				}
