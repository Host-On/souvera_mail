...
class Domain implements \JsonSerializable
{
	...
	private \Smail\Mail\Imap\Settings $IMAP;

	private \Smail\Mail\Smtp\Settings $SMTP;

	private \Smail\Mail\Sieve\Settings $Sieve;
	...
	{
//		$this->Name = \Smail\Engine\IDN::toAscii($sName);
		$this->Name = \strtolower(\idn_to_ascii($sName));
		$this->IMAP = new \Smail\Mail\Imap\Settings;
		$this->SMTP = new \Smail\Mail\Smtp\Settings;
		$this->Sieve = new \Smail\Mail\Sieve\Settings;
	}
	...
	{
//		$this->aliasName = \Smail\Engine\IDN::toAscii($sAliasName);
	...
	public function ValidateWhiteList(string $sEmail) : bool
	{
		...
		$sEmail = \Smail\Engine\IDN::emailToAscii(\mb_strtolower($sEmail));
		$iPos = \strrpos($sEmail, '@');
		$sUserPart = \substr($sEmail, 0, $iPos);
		$sUserDomain = \substr($sEmail, $iPos);
		$sItem = \strtok($sW, " ;,\n");
		while (false !== $sItem) {
			$sItem = \Smail\Engine\IDN::emailToAscii(\mb_strtolower(\trim($sItem)));
	...
	public function ImapSettings() : \Smail\Mail\Imap\Settings
	{
		return $this->IMAP;
	}

	public function SieveSettings() : \Smail\Mail\Sieve\Settings
	{
		return $this->Sieve;
	}

	public function SmtpSettings() : \Smail\Mail\Smtp\Settings
	{
		return $this->SMTP;
	}
	...
	public static function fromArray(string $sName, array $aDomain) : ?self
	{
		...
		if (!empty($aDomain['IMAP'])) {
			$oDomain->IMAP = \Smail\Mail\Imap\Settings::fromArray($aDomain['IMAP']);
			$oDomain->SMTP = \Smail\Mail\Smtp\Settings::fromArray($aDomain['SMTP']);
			$oDomain->Sieve = \Smail\Mail\Sieve\Settings::fromArray($aDomain['Sieve']);
