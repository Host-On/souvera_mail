...
trait Attachments
{
	...
	public function DoAttachmentsActions() : array
	{
		...
		$sFilename = \Smail\Mail\Base\Utils::SecureFileName($this->GetActionParam('filename', ''));
		...
		switch (\strtolower($sAction))
		{
			case 'zip':
				...
				$sZipHash = \Smail\Mail\Base\Utils::Sha1Rand();
				...
				if (!empty($sZipFileName)) {
					if (\class_exists('ZipArchive')) {
						...
						$oZip->setArchiveComment('Smail/'.APP_VERSION);
...
/*
						...
						@\unlink($sZipFileName);
						$oZip = new \Smail\Engine\Stream\ZIP($sZipFileName);
//						$oZip->setArchiveComment('Smail/'.APP_VERSION);
						foreach ($aData as $aItem) {
							if ($aItem['fileHash']) {
								$sFullFileNameHash = $oFilesProvider->GetFileName($oAccount, $aItem['fileHash']);
...
*/
						...
						foreach ($aData as $aItem) {
							...
							if ($aItem['data']) {
								try {
									$oZip->addFromString($sFileName, \Smail\Mail\Base\Utils::UrlSafeBase64Decode($aItem['data']));
								} catch (\Throwable $e) {
									$bError = true;
								}
							} else {
			...
			default:
				$data = new \Smail\Engine\AttachmentsAction;
				$data->action = $sAction;
				$data->items = $aData;
	...
	private function getMimeFileByHash(\Smail\Engine\Model\Account $oAccount, string $sHash) : array
	{
		...
		if (isset($aValues['data'])) {
			$aValues['data'] = \Smail\Mail\Base\Utils::UrlSafeBase64Decode($aValues['data']);
			...
			$this->MailClient()->MessageMimeStream(
				...
				use ($oAccount, $oFileProvider, &$aValues) {
					...
					if (\is_resource($rResource)) {
						...
						$sHash = \Smail\Mail\Base\Utils::Sha1Rand($aValues['fileName'].'~'.$sContentTypeIn);
						$rTempResource = $oFileProvider->GetFile($oAccount, $sHash, 'wb+');
						if (\is_resource($rTempResource)) {
							if (-1 < \Smail\Mail\Base\Utils::WriteStream($rResource, $rTempResource)) {
								$sResultHash = $sHash;
