...
abstract class Autoconfig
{
	...
	{
//		$emailaddress = \Smail\Engine\IDN::emailToAscii($emailaddress);
		$domain = \Smail\Mail\Base\Utils::getEmailAddressDomain($emailaddress);
//		$domain = \Smail\Engine\IDN::toAscii($domain);
...
//			?: self::mx($domain, $emailaddress)
	...
	{
		$oCache = \Smail\Engine\Api::Actions()->Cacher();
	...
	private static function mx(string $domain, string $emailaddress) : ?array
	{
		...
		foreach (\Smail\Engine\DNS::MX($domain) as $hostname) {
	...
	{
		$srv = \Smail\Engine\DNS::SRV('_submissions._tcp.'.$domain);
		if (empty($srv[0]['target'])) {
			$srv = \Smail\Engine\DNS::SRV('_submission._tcp.'.$domain);
		}
		if (!empty($srv[0]['target'])) {
			$result = [
				'incomingServer' => [],
			...
			$srv = \Smail\Engine\DNS::SRV('_imaps._tcp.'.$domain);
			if (empty($srv[0]['target'])) {
				$srv = \Smail\Engine\DNS::SRV('_imap._tcp.'.$domain);
			}
			if (!empty($srv[0]['target'])) {
	...
	{
		foreach (\Smail\Engine\DNS::SRV("_autodiscover._tcp.{$domain}") as $record) {
			if (443 == $record['port']) {
