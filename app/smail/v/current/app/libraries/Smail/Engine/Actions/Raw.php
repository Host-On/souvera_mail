...
trait Raw
{
			...
			return $this->MailClient()->MessageMimeStream(
				function ($rResource) use ($sRawKey) {
					if (\is_resource($rResource)) {
						...
						\Smail\Mail\Base\Utils::FpassthruWithTimeLimitReset($rResource);
					}
				},
	...
	public function RawUserBackground() : bool
	{
		...
		$mData = $this->StorageProvider()->Get($oAccount,
			\Smail\Engine\Providers\Storage\Enumerations\StorageType::CONFIG->value,
			'background'
		);
	...
	private function clearFileName(string $sFileName, string $sContentType, string $sMimeIndex, int $iMaxLength = 250): string
	{
		...
		$sClearedFileName = \Smail\Mail\Base\Utils::StripSpaces(\preg_replace('/[\.]+/', '.', $sFileName));
		$sExt = \Smail\Mail\Base\Utils::GetFileExtension($sClearedFileName);
		...
		return \Smail\Mail\Base\Utils::SecureFileName(\Smail\Mail\Base\Utils::Utf8Clear($sClearedFileName));
	}
	...
	private function rawSmart(bool $bDownload, bool $bThumbnail = false) : bool
	{
		...
		$sRange = \Smail\Mail\Base\Http::GetHeader('Range');
		$aMatch = array();
		...
		if (!empty($aValues['fileHash'])) {
			...
			if ('.pdf' === \substr($sFileNameIn,-4)) {
				...
				$sContentTypeOut = $sContentTypeIn
					?: \Smail\Engine\File\MimeType::fromFilename($sFileNameIn)
					?: 'application/octet-stream';
			}
			$sFileNameOut = $this->clearFileName($sFileNameIn, $sContentTypeIn, $sMimeIndex);
			\header('Content-Type: '.$sContentTypeOut);
			\Smail\Mail\Base\Http::setContentDisposition('attachment', ['filename' => $sFileNameOut]);
					...
					if (!$bDownload) {
						...
						try
						{
							if ($bThumbnail) {
								...
								\Smail\Mail\Base\Http::setContentDisposition('inline', ['filename' => $sFileName.'_thumb60x60.png']);
								$oImage->show('png');
//								$oImage->show('webp'); // Little Britain: "Safari says NO"
								...
								\Smail\Mail\Base\Http::setContentDisposition('inline', ['filename' => $sFileName]);
								$oImage->show();
//								$oImage->show('webp'); // Little Britain: "Safari says NO"
								exit;
							}
						...
						catch (\Throwable $oException)
						{
							...
							\Smail\Mail\Base\Http::StatusHeader(500);
							exit;
						}
					}
					...
					if (!\headers_sent()) {
						...
						\Smail\Mail\Base\Http::setContentDisposition($bDownload ? 'attachment' : 'inline', ['filename' => $sFileName]);
					...
					if ($sLoadedData) {
						if ($bIsRangeRequest && (\strlen($sRangeStart) || \strlen($sRangeEnd))) {
							...
							\Smail\Mail\Base\Http::StatusHeader(206);
	...
	private static function loadImage($resource, bool $bDetectImageOrientation = false, int $iThumbnailBoxSize = 0) : ?\Smail\Engine\Image
	{
		...
		$handler = 'Smail\\Engine\\Image\\'.$handler.'::createFromStream';
