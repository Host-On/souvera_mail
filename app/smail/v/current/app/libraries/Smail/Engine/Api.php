...
//				\ini_set('display_errors', '1');
			...
			\Smail\Mail\Config::$BoundaryPrefix = \trim($oConfig->Get('labs', 'boundary_prefix', ''));
		}
		return $oConfig;
	}

	public static function getCSP(?string $sScriptNonce = null) : \Smail\Engine\HTTP\CSP
	{
		$oConfig = static::Config();
		$CSP = new \Smail\Engine\HTTP\CSP(\trim($oConfig->Get('security', 'content_security_policy', '')));
		$CSP->report = $oConfig->Get('security', 'csp_report', false);
		$CSP->report_only = $oConfig->Get('debug', 'enable', false); // || SMAIL_DEV
	...
	public static function Logger() : \Smail\Mail\Log\Logger
	{
		static $oLogger = null;
		if (!$oLogger) {
			$oConfig = static::Config();
			$oLogger = new \Smail\Mail\Log\Logger(true);
			...
			if ($oConfig->Get('debug', 'enable', false)) {
				...
				$oLogger->SetLevel(\max(3, \Smail\Engine\Api::Config()->Get('logs', 'level', \LOG_WARNING)));
			}
		}
		return $oLogger;
	}
	...
	{
		$sSsoHash = \Smail\Mail\Base\Utils::Sha1Rand(\sha1($sPassword.$sEmail));

		return static::Actions()->Cacher()->Set(
			KeyPathHelper::SsoCacherKey($sSsoHash),
			\Smail\Engine\Crypt::EncryptToJSON(array(
				'Email' => $sEmail,
				'Password' => $sPassword,
