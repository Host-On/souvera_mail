...
use Smail\Engine\PGP\Backup;
use Smail\Engine\PGP\Keyservers;
use Smail\Engine\PGP\GnuPG;
...
trait Pgp
{
	 ...
	 */
	public function GnuPG() : ?\Smail\Engine\PGP\PGPInterface
	{
		...
		$homedir = \rtrim($this->StorageProvider()->GenerateFilePath(
			$oAccount,
			\Smail\Engine\Providers\Storage\Enumerations\StorageType::ROOT->value
		), '/') . '/.gnupg';

		\Smail\Mail\Base\Utils::mkdir($homedir);
		...
		if (80 < \strlen($homedir)) {
			...
			$tmpdir = \sys_get_temp_dir() . '/smail';
//			if (\Smail\Engine\Utils::inOpenBasedir($tmpdir) &&
			...
			if (80 < \strlen($homedir)) {
				...
				if ($oAccount instanceof \Smail\Engine\Model\AdditionalAccount) {
...
//				if (\Smail\Engine\Utils::inOpenBasedir($tmpdir) &&
	...
	public function DoGnupgDecrypt() : array
	{
		...
		$oPassphrase = new \Smail\Engine\SensitiveString($this->GetActionParam('passphrase', ''));
...
//			$oPart = \Smail\Mail\Mime\Part::FromString($result);
		} else {
...
//						$oPart = \Smail\Mail\Mime\Part::FromStream($rStreamHandle);
					} catch (\Throwable $e) {
						$result = $e;
					}
				},
...
//			$result['signatures'] = $oPart->SubParts[0];
	...
	public function DoGnupgExportKey() : array
	{
		$oPassphrase = $this->GetActionParam('isPrivate', '')
			? new \Smail\Engine\SensitiveString($this->GetActionParam('passphrase', ''))
			: null;
		$GPG = $this->GnuPG();
...
//			$sMicAlg = $this->GetActionParam('micAlg', '');
			...
			if ($sSigPartId) {
				...
				$decode = (new \Smail\Mail\Mime\HeaderCollection($sBodyMime))->ValueByName(MimeEnumHeader::CONTENT_TRANSFER_ENCODING->value);
				if ('base64' === $decode) {
