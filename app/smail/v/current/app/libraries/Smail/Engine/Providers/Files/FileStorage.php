...
class FileStorage implements \Smail\Engine\Providers\Files\IFiles
{
	/**
	 * @var array
	 */
	...
	public function PutFile(\Smail\Engine\Model\Account $oAccount, string $sKey, /*resource*/ $rSource) : bool
	{
		...
		if ($rSource) {
			...
			if ($rOpenOutput) {
				$bResult = (false !== \Smail\Mail\Base\Utils::WriteStream($rSource, $rOpenOutput));
				\fclose($rOpenOutput);
			}
		}
		return $bResult;
	...
	public function GetFileName(\Smail\Engine\Model\Account $oAccount, string $sKey) /*: string|false*/
	{
	...
	public function FileSize(\Smail\Engine\Model\Account $oAccount, string $sKey) /*: int|false*/
	{
		$sFileName = $this->generateFullFileName($oAccount, $sKey);
		...
		if (0 < $iTimeToClearInHours) {
			...
			foreach (\glob("{$this->sDataPath}/*", GLOB_ONLYDIR) as $sDomain) {
				foreach (\glob("{$sDomain}/*", GLOB_ONLYDIR) as $sLocal) {
					\Smail\Mail\Base\Utils::RecTimeDirRemove("{$sLocal}/.files", $iTimeToClear);
				}
			}
			// Old
			\Smail\Mail\Base\Utils::RecTimeDirRemove("{$this->sDataPath}/files", $iTimeToClear);
	...
	private function generateFullFileName(\Smail\Engine\Model\Account $oAccount, string $sKey, bool $bMkDir = false) : string
	{
		...
		$sFilePath = $this->sDataPath
			...
			.($sSubEmail ? '/'.\Smail\Mail\Base\Utils::SecureFileName($sSubEmail) : '')
			.'/.files/'.\sha1($sKey);

		$bMkDir && \Smail\Mail\Base\Utils::mkdir(\dirname($sFilePath));
