...
//				\Smail\Mail\Base\Utils::mkdir($sNewDir)
				if (\is_dir($sNewDir) || \mkdir($sNewDir, 0700, true)) {
					...
					\Smail\Mail\Base\Utils::RecRmDir($sDomainDir);
				}
			}
		}
		\Smail\Mail\Base\Utils::RecRmDir("{$sDataPath}/cfg");
		\Smail\Mail\Base\Utils::RecRmDir("{$sDataPath}/data");
		\Smail\Mail\Base\Utils::RecRmDir("{$sDataPath}/files");
	}

	/**
	 * Attempt to convert the old less secure data into better secured data
	 */
	public static function ConvertInsecureAccounts(\Smail\Engine\Actions $oActions, \Smail\Engine\Model\MainAccount $oMainAccount) : array
	{
		...
		if (1 < \count($aAccounts)) {
			...
			foreach ($aAccounts as $sEmail => $sToken) {
				...
				try {
					...
					if (!$sToken) {
						\Smail\Engine\Log::warning('UPGRADE', "ConvertInsecureAccount {$sEmail} no token");
						continue;
					}
					...
					if (empty($aAccountHash[0]) || 'token' !== $aAccountHash[0] // simple token validation
						...
						\Smail\Engine\Log::warning('UPGRADE', "ConvertInsecureAccount {$sEmail} invalid aAccountHash: " . \print_r($aAccountHash,1));
						continue;
					}
					...
					\Smail\Engine\Log::warning('UPGRADE', "ConvertInsecureAccount {$sEmail} failed");
				}
			}
	...
	{
		$sData = \Smail\Mail\Base\Utils::UrlSafeBase64Decode($sData);
		if (!\strlen($sData)) {
			return [];
		}
		$sKey = \md5(APP_SALT);
		$sData = \is_callable('xxtea_decrypt')
			...
			: \Smail\Mail\Base\Xxtea::decrypt($sData, $sKey);
		try {
...
//		$tar_destination = APP_DATA_FOLDER_PATH . APP_VERSION . '.tar';
		...
		if (\class_exists('PharData')) {
			...
			$tar_destination .= '.tgz';
			$tar = new \Smail\Engine\Stream\TAR($tar_destination);
		}
		...
		if ($tar instanceof \Smail\Engine\Stream\TAR) {
	...
	// Smail: in-app core update removed — managed via NC app store / CI
	...
	public static function fixPermissions($mode = 0755) : void
	{
		...
		foreach (\glob("{$target}/smail/v/*", \GLOB_ONLYDIR) as $dir) {
			\chmod($dir, 0755);
