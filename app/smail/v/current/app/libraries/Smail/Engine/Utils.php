...
class Utils
{
	...
	{
		try {
			/* Issue with \Smail\Engine\HTTP\Stream
	...
	{
		return \Smail\Engine\Crypt::EncryptUrlSafe(
			$aValues,
	...
	{
		return \Smail\Engine\Crypt::DecryptUrlSafe(
			$sEncodedValues,
	...
	{
		$sToken = \Smail\Engine\Cookies::get(self::SESSION_TOKEN);
		if (!$sToken) {
			if (!$generate) {
				return null;
			}
			\Smail\Engine\Log::debug('TOKENS', 'New SESSION_TOKEN');
			$sToken = \Smail\Mail\Base\Utils::Sha1Rand(APP_SALT);
			\Smail\Engine\Cookies::set(self::SESSION_TOKEN, $sToken);
		}
...
//		$oAccount = $oActions->getMainAccountFromToken(false);
		...
		if (!$sSeed) {
			...
			$sSeed = $sFallback ??= \Smail\Mail\Base\Utils::Sha1Rand(APP_SALT);
		}
...
//		return $sHtml;
	...
	{
		// Smail: use static 'current' path — version is in info.xml, not the directory name
		return self::WebPath() . 'smail/v/current/';
	}
...
//			\Smail\Engine\Log::warning('OpenBasedir', "open_basedir restriction in effect. {$name} is not within the allowed path(s): " . \ini_get('open_basedir'));
			return false;
		}
		return true;
	}
