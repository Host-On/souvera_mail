...
class Domain extends AbstractProvider
{
	...
	private \Smail\Engine\Plugins\Manager $oPlugins;

	public function __construct(Domain\DomainInterface $oDriver, \Smail\Engine\Plugins\Manager $oPlugins)
	{
	...
	public function SaveAlias(string $sName, string $sAlias) : bool
	{
		if ($this->Load($sName, false, false)) {
			throw new ClientException(\Smail\Engine\Notifications::DomainAlreadyExists->value);
		}
	...
	public function LoadOrCreateNewFromAction(\Smail\Engine\Actions $oActions, ?string $sNameForTest = null) : ?\Smail\Engine\Model\Domain
	{
		...
		if (\strlen($sName) || $sNameForTest) {
			...
			return \Smail\Engine\Model\Domain::fromArray($sNameForTest ?: $sName, [
	...
	public function getByEmailAddress(string $sEmail) : \Smail\Engine\Model\Domain
	{
		$oDomain = $this->Load(\Smail\Mail\Base\Utils::getEmailAddressDomain($sEmail), true);
		if (!$oDomain) {
