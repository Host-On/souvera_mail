...
class Suggestions extends \Smail\Engine\Providers\AbstractProvider
{
	/**
	 * @var \Smail\Engine\Providers\Suggestions\ISuggestions[]
	 */
	private array $aDrivers = [];

	/**
	 * @param \Smail\Engine\Providers\Suggestions\ISuggestions[]|null $aDriver = null
	 */
	public function __construct(?array $aDriver = null)
	{
		if (\is_array($aDriver)) {
			$this->aDrivers = \array_filter($aDriver, function ($oDriver) {
				return $oDriver instanceof \Smail\Engine\Providers\Suggestions\ISuggestions;
			});
		}
	}
	...
	public function Process(\Smail\Engine\Model\Account $oAccount, string $sQuery, int $iLimit = 20) : array
	{
		...
		try
		{
			$oAddressBookProvider = \Smail\Engine\Api::Actions()->AddressBookProvider($oAccount);
