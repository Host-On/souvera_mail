...
use Smail\Engine\Providers\AddressBook\AddressBookInterface;
use Smail\Engine\Providers\AddressBook\Classes\Contact;
...
class NextcloudAddressBook implements AddressBookInterface
{
	...
	public function ContactSave(Contact $oContact): bool
	{
		...
		if (empty((string) $vCard->UID)) {
			$vCard->UID = \Smail\Engine\UUID::generate();
		}
	...
	private function contactToVCard(array $ncContact): ?VCard
	{
		...
		$vCard = new VCard();
		$vCard->UID = $ncContact['UID'] ?? \Smail\Engine\UUID::generate();
		$fn = $ncContact['FN'] ?? '';
		$vCard->FN = $fn;

		// Smail displays the N (structured name) property.
	...
	{
		$props = [];
		$props['UID'] = (string) ($vCard->UID ?? \Smail\Engine\UUID::generate());
