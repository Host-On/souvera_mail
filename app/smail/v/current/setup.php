...
require_once __DIR__ . '/app/libraries/Smail/Engine/integrity.php';

$result = \Smail\Engine\Integrity::phpVersion();
if ($result) {
	echo '<p style="color: red">[301] ' . $result . '</p>';
	exit(301);
}

$result = \Smail\Engine\Integrity::phpExtensions();
if ($result) {
...
//			break;
	...
	if (defined('SMAIL_UPDATE_PLUGINS')) {
		// Update plugins
		$asApi = !empty($_ENV['SMAIL_INCLUDE_AS_API']);
		$_ENV['SMAIL_INCLUDE_AS_API'] = true;
		$aList = \Smail\Engine\Repository::getEnabledPackagesNames();
		foreach ($aList as $sId) {
			\Smail\Engine\Repository::installPackage('plugin', $sId);
		}
		$_ENV['SMAIL_INCLUDE_AS_API'] = $asApi;
	}
