    ...
    private function refreshOidcDirectories(array &$outerReport): array
    {
        ...
        if (empty($allIds)) {
            return [
                'ok' => false,
                'touched' => [],
                'error' => 'Stalwart has no directories at all — run the deploy-agent bootstrap step that creates the Nextcloud OIDC directory',
            ];
        }
        ...
        if (empty($oidcIds)) {
            return [
                'ok' => false,
                'touched' => [],
                'error' => 'no OIDC-typed Directory found in Stalwart (only found: ' . \implode(', ', $allIds) . ') — run the deploy-agent bootstrap step that creates the Nextcloud OIDC directory',
            ];
        }
