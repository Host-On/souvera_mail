 ...
 * 3. Pre-loads the `lib-bridge/Souvera_mail/AppInfo/Application.php`
 *    concrete bridge subclass so NC's DI container has a real class to
 *    `new` when it tries to boot the app via the underscore namespace.
 *
 * Re-entrancy guard
 * -----------------
...
(static function (): void {
    // composer/autoload.php → app-root → lib/, lib-bridge/
    $appRoot = \dirname(__DIR__);
    $libDir = $appRoot . '/lib/';
    $bridgeDir = $appRoot . '/lib-bridge/';
    ...
    if (isset(\OC::$composerAutoloader) && \is_object(\OC::$composerAutoloader)) {
        try {
            ...
            // Bridge subclasses for the underscore variant live under
            // lib-bridge/Souvera_mail/ — register that prefix too so the
            // concrete `OCA\Souvera_mail\AppInfo\Application` class resolves.
            \OC::$composerAutoloader->addPsr4('OCA\\Souvera_mail\\', $bridgeDir . 'Souvera_mail/', false);
        } catch (\Throwable $e) {
    ...
    //    when they ship.
    \spl_autoload_register(static function (string $class) use ($bridgeDir): void {
        static $prefix = 'OCA\\Souvera_mail\\';
        if (!\str_starts_with($class, $prefix)) {
            return;
        }
        $relative = \substr($class, \strlen($prefix));
        $bridgeFile = $bridgeDir . 'Souvera_mail/' . \strtr($relative, '\\', \DIRECTORY_SEPARATOR) . '.php';
